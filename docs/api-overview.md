...
(theft detection) and returns 401. Forgot-password always answers identically;
the reset link is delivered by SMTP only.

## Users (`/api/users`, org-admin unless noted)
...
`DELETE` (blocked while users assigned).

## Recognition (`/api/recognize`)

```
POST  /api/recognize                       single frame {image_base64, threshold,
                                            check_liveness, auto_mark_attendance}
POST  /api/recognize/batch                 up to 10 frames
