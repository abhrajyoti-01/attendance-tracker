...
1. `POST /api/users/{id}/register` with 3–5 base64 JPEG/PNG frames
   (org-admin token). Poor-quality frames are rejected with per-category counts.
2. `POST /api/recognize` with a frame + API key or user token.
...
| recognition always misses | model file missing (run step 2) or embeddings stored by a different-dimension model — rebuild indexes via the beat task |
